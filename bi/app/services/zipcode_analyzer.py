...
_CONT_DEFAULTS: dict[str, float] = {
    "sqft": 1000.0,
    "bedrooms": 2.0,
    "bathrooms": 1.0,
    "year_built": 1960.0,
    "walk_score": 50.0,
    "walk_score_resid": 0.0,
    "transit_score": 30.0,
    "amenity_count_1km": 5.0,
    "median_income": 75000.0,
    "months_since_2022_q1": 0.0,
    "months_since_2022_q1_sq": 0.0,
}
...
def _fill_cont_defaults(cont: dict[str, float]) -> dict[str, float]:
    ...
    for k, v in list(out.items()):
        if v is None or (isinstance(v, float) and math.isnan(v)):
            out[k] = _CONT_DEFAULTS.get(k, 0.0)
    return out
...
async def _enrich_model_or_hybrid(
    ...
    if overall_label in ("low", "medium"):
        warnings.append(WARN_MODEL_DOM_LOW_CONFIDENCE)
    if total_n < 100 or overall_label == "low":
